...
def test_help(options: list[str]) -> None:
    ...
    assert result.output.startswith("Usage: "), f"Help output for {options} did not start with 'Usage:'"
...
def test_version(options: list[str]) -> None:
    ...
    assert result.exit_code == 0, f"Version options {options} failed with exit code {result.exit_code}"
    assert __version__ in result.output, f"Expected version {__version__} not found in output for {options}"
